...
/**
 * Modal para crear/editar tipos de cuenta
 */
const TipoCuentaModal = ({ 
  ...
  const isEditing = Boolean(tipoCuenta)

  // Resetear formulario cuando se abre/cierra el modal
  useEffect(() => {
    if (isOpen) {
      if (isEditing && tipoCuenta) {
        setNombre(tipoCuenta.name || '')
      } else {
        setNombre('')
      }
      setErrors({})
    }
  }, [isOpen, isEditing, tipoCuenta])
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? 'Editar Tipo de Cuenta' : 'Nuevo Tipo de Cuenta'}
      size="small"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Ej: Banco, Efectivo, Tarjeta de Crédito"
            disabled={isLoading}
            autoFocus
          />
          {errors.nombre && (
